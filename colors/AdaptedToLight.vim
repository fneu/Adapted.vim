" {{{ Setup
  ...
  let g:colors_name="AdaptedToLight"
" }}}
" {{{ Italics
  let g:adapted_terminal_italic = get(g:, 'adapted_terminal_italic', 0)
  let s:italic = ""
  if g:adapted_terminal_italic == 1
    let s:italic = "italic"
  endif
" }}}
" {{{ Bold
  let g:adapted_terminal_bold = get(g:, 'adapted_terminal_bold', 0)
  let s:bold = ""
  if g:adapted_terminal_bold == 1
   let s:bold = "bold"
  endif
" }}}
" {{{ Colors
  let s:base00=['#eceff1', '253']
  let s:base01=['#cfd8dc', '252']
  let s:base02=['#b0bec5', '251']
  let s:base03=['#90a4ae', '145']
  let s:base04=['#546e7a', '243']
  let s:base05=['#455a64', '240']
  let s:base06=['#37474f', '237']
  let s:base07=['#263238', '235']
  let s:base08=['#D32F2F', '124']
  let s:base09=['#F57C00', '131']
  let s:base0A=['#FBC02D', '137']
  let s:base0B=['#388E3C', '101']
  let s:base0C=['#00796B', '72']
  let s:base0D=['#1976D2', '60']
  let s:base0E=['#7B1FA2', '96']
  let s:base0F=['#5D4037', '101']
...
call <sid>hi('pythonDocstring',            s:base03, '',       s:italic,    '')
